...
```bash
pip install -r requirements.txt   # включает watchdog
python autosave.py
```

Игнорируются: `.git`, `__pycache__`, `venv`, `.env`, `*.log`, `*.db`.
Между изменениями выдерживается пауза 5 секунд (`DEBOUNCE_SECONDS` в
`autosave.py`), чтобы не плодить коммит на каждый чих при массовом
сохранении файлов. Останов — `Ctrl+C`.

Запускать нужно из папки, где уже настроен git-remote (репозиторий уже
склонирован или инициализирован и подключён к GitHub).
